...
const toCsv = (csvLines = [[]]) => fs.writeFile(
	"kult-tarot-data.csv",
	`data:text/csv;charset=utf-8,${csvLines
		.map((line) => line
...
for (const cardData of TAROTDATA["major-arcana"]) {
	for (const [cat, texts] of Object.entries(cardData.ghostText)) {
		cardData[`gt-${cat}`] = texts.join("|");
	}
	delete cardData.ghostText;
	delete cardData.isDrawingShiftCard;
	if (csvLines.majorArcana.length === 0) {
		csvLines.majorArcana.push(...[
			["MAJOR ARCANA"],
			Object.keys(cardData)
		]);
	}
	csvLines.majorArcana.push(Object.values(cardData));
}

for (const [suit, suitData] of Object.entries(TAROTDATA["minor-arcana"])) {
	for (const cardData of suitData.slice(1)) {
		for (const [cat, texts] of Object.entries(cardData.ghostText)) {
			cardData[`gt-${cat}`] = texts.join("|");
		}
		delete cardData.ghostText;
		if (csvLines.minorArcana.length === 0) {
			csvLines.minorArcana.push(...[
				[""],
				[""],
				["MINOR ARCANA"],
				Object.keys(cardData)
		...
		csvLines.minorArcana.push(Object.values(cardData));
	}
}

toCsv([...csvLines.majorArcana, ...csvLines.minorArcana]);
