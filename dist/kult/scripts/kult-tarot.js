...
const CONSTANTS = {
	...
	timing: {
		...
		cardTextFadeInTime: 15000,
		cardTextFadeOutTime: 15000,
		cardTextDuration: {
			min: 1000,
			max: 1000
		},
		...
		crescents: [
			...
			},
			{
				...
				details: {
					individual: [
						"Greedy fixer",
						"Silver tongued lobbyist",
						"Irritated network technician",
						"Femme fatale with a harem of lovers",
						"Street contact",
						"Nosey electrician",
						"Charismatic club owner",
						"Sect leader"
					],
					location: [
						"Group of islands",
			...
			},
			{
				...
				details: {
					individual: [
						"Good-hearted priest",
						"Conjoined twins",
						"Manager at the acquisition branch of a company",
						"Charismatic cult leader",
						"Married couple acting in unison"
					],
					location: [
						"Where two rivers connect",
					...
					situation: [
						...
						"Minority assimilated into society",
						"Myth and reality become one",
						"Two ideologies become one",
						"Two people prove to be the same"
					],
					creature: [
						"Cairath",
						"Being of Passion"
					...
					item: [
						...
						"A cross with a hidden blade",
						"Vial with blood from twelve children"
					]
				},
			...
			},
			{
				...
				details: {
					individual: [
						...
						"Amature filmmaker",
						"Hard working mime",
						"Burned out performance artist",
						"Honest singer songwriter",
						"Well meaning Cognitive-behavior therapist"
					],
					location: [
						"Still pond",
						"Carnival",
			...
			},
			{
				...
				details: {
					individual: [
						"Postman who reads the mail",
						"Phone sex operator",
						"Overwintered lecturer",
						"TV-addicted shut-in",
						"Janitor working the night shift",
						"Suicidal toll booth operator",
						"Mediocre stand-up comedian with two good jokes",
						"Sleepwalker"
					],
					location: [
						"Marriage chapel",
						"Hair and nail salon",
						"Government office",
						"Cheap night club with the same act night after night",
						"Amusement park with the same jingles and rides from 20 years ago"
					],
					organization: [
						"Conservative think tank",
						"Pharmaceutical company supplying the middle class with antidepressants"
					],
					situation: [
						"Cover band playing the same set of songs night after night",
						"Living on old merits",
						"Telling the same story again and again",
						"Déjà vu",
						"Having to go through the same procedure once more"
					],
					creature: [
						"Acrotide",
						"Tekron",
						"Nearly mindless spectre"
					],
					item: [
						"Hamster wheel",
						"Turned down script",
						"Music box that plays the same melody"
					]
				},
			...
			},
			{
				...
				details: {
					individual: [
						"Calm and seductive guru",
						"Chain smoking housewife who has given up",
						"Old hippie who speaks in riddles"
					],
					location: [
						"Sleepy small town frozen in time",
						"Suburban houses along a cul de sac",
						"Japanese garden",
						"Slow flowing river on the pastoral countryside",
						"Ghost town",
						"Abandoned drive-in theater"
					],
					organization: [
						"Meditation centre",
						"Research centre mapping human emotions"
					],
					situation: [
						"A calm and quiet day",
						"School reunion where everyone is exactly as before",
						"Strict family dinner"
					],
					creature: [
						"Lost god that has forgotten its nature"
					],
					item: [
						"Sleeping pills",
						"CD with calm and harmonic music",
						"A painting of a desert landscape",
						"Book with 12 steps to control one's aggression",
						"Panpipe"
					]
				},
...
const updateSlotOpacity = (slot, startSlot) => {
	console.log(`updateSlotOpacity(${slot}, ${startSlot})`);
	const {mainCard, isFaceUp} = LAYOUT[slot];
	const $slot = $(`#slot-${slot}`);
	const curOpacity = $slot.css("opacity");
	 if (Number.isInteger(startSlot)) {
		if (!mainCard || isFaceUp || slot === 1 || slot === startSlot) {
			console.log([
				!mainCard,
				Boolean(isFaceUp),
				slot === 1,
			...
			LAYOUT[slot].opacity = 1;
		} else {
			const posInQueue = slot - startSlot;
			LAYOUT[slot].opacity = 0.5 - (0.1 * posInQueue);
		}
		const fadeDuration = 2500 * Math.abs(curOpacity - LAYOUT[slot].opacity);
		console.log(`Slot ${slot}, ${curOpacity} -> ${LAYOUT[slot].opacity}, Dur: ${fadeDuration}`);
		$(`#slot-${slot}`).fadeTo(fadeDuration, LAYOUT[slot].opacity);
	} else {
		let thisSlot = slot;
		while (thisSlot <= 5) {
			updateSlotOpacity(thisSlot, slot);
			thisSlot++;
		}
	}
};
const updateSlotImage = (slot) => {
	...
	if (mainCard) {
		if (isFaceUp) {
			main$.css("--bg-image", getImgStyle(mainCard.imgfile));
			main$.css("filter", "drop-shadow(0 0 10px red)");
		} else {
			main$.css("--bg-image", getImgStyle(CONSTANTS.bgCardBack));
			if (slot === 1 || LAYOUT[slot - 1].isFaceUp) {
				main$.css("filter", "drop-shadow(0 0 10px red)");
			} else {
				main$.css("filter", "drop-shadow(0 0 10px red) blur(10px)");
			}
		}
	} else {
		main$.css("--bg-image", getImgStyle(CONSTANTS.bgEmptySlot));
		main$.css("filter", "drop-shadow(0 0 10px red)");
	}
	if (shiftCard) {
		shift$.removeClass("hidden");
		shift$.css("--bg-image", getImgStyle(shiftCard.imgfile));
	} else {
		shift$.addClass("hidden");
		shift$.css("--bg-image", getImgStyle(CONSTANTS.bgEmptySlot));
	}
};
...
const updateSlotText = (slot, isResettingInterval = true) => {
	if (Number.isInteger(slot)) {
		...
		const {mainCard, shiftCard, isFaceUp} = LAYOUT[slot];
		const [$mainTitle, $shiftTitle] = [
			$(`#card-${slot}-title`),
			$(`#card-${slot}-shiftTitle`)
		];
		const [$mainKeyword, $shiftKeyword] = [
			$(`#card-${slot}-keyword`),
			$(`#card-${slot}-shiftKeyword`)
		];
		if (mainCard && isFaceUp) {
			...
			if (shiftCard && "suit" in shiftCard) {
				LAYOUT[slot].fadeText.push(...Object.entries(READINGDATA.textCategories)
					.filter(([category, isActive]) => isActive)
				...
				$shiftKeyword.fadeOut(200, () => {
					$shiftKeyword.text(shiftCard.keyword);
					$shiftKeyword.fadeIn(200);
				});
			} else {
				$shiftTitle.fadeOut(200, () => {
					$shiftTitle.text("");
...
const dealCard = (slot, isShifting = false) => {
	if (Number.isInteger(slot)) {
